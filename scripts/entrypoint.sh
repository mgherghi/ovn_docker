...
if [[ "${ROLE}" == "bootstrap" ]]; then
  if [[ ! -f "$NB_DB" ]]; then
    ...
    ovsdb-tool create-cluster "$NB_DB" /usr/share/ovn/ovn-nb.ovsschema "tcp:${ENCAP_IP}:6641"
  fi
  if [[ ! -f "$SB_DB" ]]; then
    log "Creating SB RAFT cluster (bootstrap @ ${ENCAP_IP}:6642)"
    ovsdb-tool create-cluster "$SB_DB" /usr/share/ovn/ovn-sb.ovsschema "tcp:${ENCAP_IP}:6642"
  fi
else
  : "${BOOTSTRAP_IP:?BOOTSTRAP_IP required for join role}"
  if [[ ! -f "$NB_DB" ]]; then
    log "Joining NB RAFT cluster @ ${BOOTSTRAP_IP}:6641"
    ovsdb-tool join-cluster "$NB_DB" /usr/share/ovn/ovn-nb.ovsschema "tcp:${BOOTSTRAP_IP}:6641"
  fi
  if [[ ! -f "$SB_DB" ]]; then
    log "Joining SB RAFT cluster @ ${BOOTSTRAP_IP}:6642"
    ovsdb-tool join-cluster "$SB_DB" /usr/share/ovn/ovn-sb.ovsschema "tcp:${BOOTSTRAP_IP}:6642"
  fi
fi
